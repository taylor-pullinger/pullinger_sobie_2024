function [dydt, ij_data] = f_ToRORd19(t, Y, celltype, universals, geometry, parameters, conductances, vclamp_V)

tau_multipliers = struct();
tau_multipliers.m = 1;
tau_multipliers.h = 1;
tau_multipliers.j = 1;
...
%% set geometry
% L = geometry(1) ; - unused
% rad = geometry(2) ; - unused
% vcell = geometry(3) ; - unused
% Ageo = geometry(4) ; - unused
Acap = geometry(5) ;
vmyo = geometry(6) ;
vnsr = geometry(7) ;
vjsr = geometry(8) ;
...
%% set parameters
ICaL_fractionSS = parameters(1) ;
INaCa_fractionSS = parameters(2) ;

%% set conductances
INa_Multiplier = conductances(1) ; 
INaL_Multiplier = conductances(2) ;
Ito_Multiplier = conductances(3) ;
ICaL_Multiplier = conductances(4) ;
IKr_Multiplier = conductances(5) ;
IKs_Multiplier = conductances(6) ;
IK1_Multiplier = conductances(7) ;
INaCa_Multiplier = conductances(8) ;
INaK_Multiplier = conductances(9) ;
IKb_Multiplier = conductances(10) ;
INab_Multiplier = conductances(11) ;
ICab_Multiplier = conductances(12) ;
IpCa_Multiplier = conductances(13) ;
ICaCl_Multiplier = conductances(14) ;
IClb_Multiplier = conductances(15) ;
Jrel_Multiplier = conductances(16) ;
Jup_Multiplier = conductances(17) ;

%% set state variables: Y
v = Y(1) ;
nai = Y(2) ;
nass = Y(3) ;
ki = Y(4) ;
...
%% INa
[INa, dm, dh, dhp, dj, djp]...
    = getINa_Grandi(v, m, h, hp, j, jp, fINap, ENa, INa_Multiplier, tau_multipliers) ;

%% INaL
[INaL, dmL, dhL, dhLp]...
    = getINaL_ORd2011(v, mL, hL, hLp, fINaLp, ENa, celltype, INaL_Multiplier) ;
...
%% ICaL
[ICaL_ss, ICaNa_ss, ICaK_ss, ICaL_i, ICaNa_i, ICaK_i, dd, dff, dfs, dfcaf, dfcas, ...
    djca, dnca, dnca_i, dffp, dfcafp, PhiCaL_ss, PhiCaL_i, gammaCaoMyo, gammaCaiMyo]...
    = getICaL_ORd2011_jt(v, d, ff, fs, fcaf, fcas, jca, nca, nca_i, ffp, fcafp, fICaLp,...
    cai, cass, cao, nai, nass, nao, ki, kss, ko, cli, clo, celltype, ICaL_fractionSS, ICaL_Multiplier) ;
...
%% IKr
[IKr, dt_ikr_c0, dt_ikr_c1, dt_ikr_c2, dt_ikr_o, dt_ikr_i ]...
    = getIKr_ORd2011_MM(v, ikr_c0, ikr_c1, ikr_c2, ikr_o, ikr_i, ko, EK, celltype, IKr_Multiplier) ;

%% IKs
[IKs,dxs1, dxs2]...
    = getIKs_ORd2011(v, xs1, xs2, cai, EKs, celltype, IKs_Multiplier);
...
%% INaK
INaK...
    = getINaK_ORd2011(v, F, R, T, nai, nao, ki, ko, celltype, INaK_Multiplier);
...
%% Jrel
[Jrel, dJrel_np, dJrel_p]...
    = getJrel_ORd2011(Jrel_np, Jrel_p, ICaL_ss, cass, cajsr, fJrelp, celltype, Jrel_Multiplier) ;

%% Jup
fJupp = (1.0 / (1.0 + KmCaMK / CaMKa)) ;
[Jup, Jleak]...
    = getJup_ORd2011(cai, cansr, fJupp, celltype, Jup_Multiplier) ;

% calculate tranlocation flux Jtr
Jtr = (cansr - cajsr) / 60 ;

% we pass Istim to the dydt function as item 44 of the state variables vector, Y 
...
if ~isempty(vclamp_V)
    ...
    dv = -(INa + INaL + Ito + ICaL + ICaNa + ICaK + IKr + IKs + IK1 + INaCa_i...
    + INaCa_ss + INaK + INab + IKb + IpCa + ICab + I_ClCa + I_Clbk - Istim);
end  
% in the original, a negative Istim is added, since we use a positive value for our Istim, it is subtracted
dstim = 0 ;
...
%% dydt & ij_data
dydt = [dv dnai dnass dki dkss dcai dcass dcansr dcajsr dm dhp dh dj djp dmL...
    dhL dhLp da diF diS dap diFp diSp dd dff dfs dfcaf dfcas djca dnca dnca_i...
    dffp dfcafp dxs1 dxs2 dJrel_np dCaMKt dt_ikr_c0 dt_ikr_c1 dt_ikr_c2 dt_ikr_o...
    dt_ikr_i dJrel_p dstim]';

ij_data = [INa INaL Ito ICaL IKr IKs IK1 INaCa_i INaCa_ss INaK  IKb INab ICab...
    IpCa Jdiff JdiffNa JdiffK Jup Jleak Jtr Jrel CaMKa Istim fINap fINaLp...
    fICaLp fJrelp fJupp cajsr cansr PhiCaL_ss v ICaL_i I_ClCa I_Clbk ICaL_tot] ;
end

%% LOCAL FUNCTIONS FOR FINDING CURRENTS

%% INa 
function [INa, dm, dh, dhp, dj, djp]...
    = getINa_Grandi(v, m, h, hp, j, jp, fINap, ENa, INa_Multiplier, tau_multipliers)
% The Grandi implementation updated with INa phosphorylation.
%% m gate
mss = 1 / ((1 + exp( -(56.86 + v) / 9.03 )) ^ 2) ;
taum = tau_multipliers.m * (0.1292 * exp(-((v + 45.79) / 15.54) ^ 2) + 0.06487 * exp(-((v - 4.823) / 51.12) ^ 2)) ;
dm = (mss - m) / taum ;
...
tauh = tau_multipliers.h * (1 / (ah + bh)) ;
...
tauj = tau_multipliers.j * (1 / (aj + bj)) ;
...
%% ITo
function [Ito, da, diF, diS, dap, diFp, diSp]...
    = getITo_ORd2011(v, a, iF, iS, ap, iFp, iSp, fItop, EK, celltype, Ito_Multiplier)
...
function [ICaL_ss, ICaNa_ss, ICaK_ss, ICaL_i, ICaNa_i, ICaK_i, dd, dff, dfs,...
    dfcaf, dfcas, djca, dnca, dnca_i, dffp, dfcafp, PhiCaL_ss, PhiCaL_i,...
...
function [IKr, dc0, dc1, dc2, do, di ]...
    = getIKr_ORd2011_MM(V, c0, c1, c2, o, i, ko, EK, celltype, IKr_Multiplier)
...
%% Jrel
function [Jrel, dJrelnp, dJrelp]...
    = getJrel_ORd2011(Jrelnp, Jrelp, ICaL, cass, cajsr, fJrelp, celltype, Jrel_Multiplier)
